...
namespace PengDongNanTools
    ...
    //TCP/IP以太网客户端通讯界面【软件作者：彭东南, southeastofstar@163.com】
    /// <summary>
    /// TCP/IP以太网客户端通讯界面【软件作者：彭东南, southeastofstar@163.com】
    /// </summary>
    public partial class TCPClientForm : Form
        ...
        private Thread ClientConnectToServerThread;

        private SimpleTCPIPClient[] Clients;
        ...
        private Thread MultiServerConnectThread;
        ...
        private void TCPClientForm_FormClosing(object sender, FormClosingEventArgs e)
            ...
            try
                ...
                if (MultiServerConnectThread != null)
                    {
                    MultiServerConnectThread.Abort();
                    MultiServerConnectThread = null;
                    }
        ...
        private void txtSendToServer_KeyDown(object sender, KeyEventArgs e)
            {
            SingleServerSend = txtSendToServer.Text;
            MultiServerSend = txtSendToServer.Text;
            }
        ...
        private void tmrAutoSend_Elapsed(object sender, System.Timers.ElapsedEventArgs e) 
            ...
            try
                ...
                if (TCPClientStation == null)
                    {
                    AddText("单端口连接客户端没有创建...");
                    }
                else
                    {
                    if (TCPClientStation.Connected == false)
                        {
                        AddText("单端口连接客户端没有与服务器建立连接...");
                        }
                    }

                if (Clients == null)
                    {
                    AddText("连接多服务器/端口的客户端没有创建...");
                    }
                ...
                if (SendFileFlag == false)
                    ...
                    if (txtSendToServer.Text != "")
                        ...
                        string TempString = "";
                        TempString = txtSendToServer.Text;
                        ...
                        if (TCPClientStation != null)
                            ...
                            if (TCPClientStream != null)
                                ...
                                if (ClientSendMessageInHEX == true)
                                    {
                                    TempString = FC.StringConvertToHEX(TempString + Suffix);
                                ...
                                AddText("客户端从本地端口 " + FC.GetLocalPort(TCPClientStation) + " 成功发送到服务器 " +
                                        TargetServerIPAddress + " 的端口： " + TargetServerPort + "，文本内容：" + TempString);
                                }
                            }
                        ...
                        if (Clients != null)
                            ...
                            for (int a = 0; a < Clients.Length; a++)
                                {
                                Clients[a].Suffix = Suffix;
                                if (ClientSendMessageInHEX == true)
                                    {
                                    Clients[a].SendMessageInHEX = true;
                                    //Clients[a].Send(txtSendToServer.Text);
                                    if (Clients[a].Send(txtSendToServer.Text) == true)
                                        {
                                        AddText("客户端 " + (a + 1) + " 从本地端口 " + Clients[a].ClientPort +
                                        " 成功发送到服务器 " + Clients[a].ServerIPAddress
                                        + " 的端口： " + Clients[a].ServerListenPort + "，文本内容：" +
                                        FC.StringConvertToHEX(txtSendToServer.Text + Clients[a].Suffix));
                                        }
                                    else
                                        {
                                        AddText("客户端 " + (a + 1) + "发送失败...");
                                        }
                                    }
                                else
                                    {
                                    if (GB2312Coding == true)
                                        {
                                        Clients[a].GB2312Coding = true;
                                        }
                                    if (Clients[a].Send(txtSendToServer.Text) == true)
                                        {
                                        AddText("客户端 " + (a + 1) + " 从本地端口 " + Clients[a].ClientPort +
                                            " 成功发送到服务器 " + Clients[a].ServerIPAddress
                                            + " 的端口： " + Clients[a].ServerListenPort + "，文本内容：" + txtSendToServer.Text); 
                                        }
                                    else 
                                        {
                                        AddText("客户端 " + (a + 1) + "发送文本内容失败");
                                        }
                        ...
                        for (int a = 0; a < newOpenFile.FileNames.Length; a++)
                            ...
                            if (TCPClientStation != null)
                                ...
                                if (TCPClientStream != null)
                                    ...
                                    try
                                        ...
                                        if (ClientSendMessageInHEX == true)
                                            {
                                            TempStr = FC.StringConvertToHEX(TempStr + Suffix);
                                            TCPClientStream.Write(System.Text.Encoding.UTF8.GetBytes(TempStr), 0,
                                                          System.Text.Encoding.UTF8.GetBytes(TempStr).Length);
                                            }
                                        else
                                            {
                                            if (GB2312Coding == true)
                                                {
                                                TCPClientStream.Write(System.Text.Encoding.GetEncoding(936).GetBytes(TempStr + Suffix), 0,
                                                    System.Text.Encoding.GetEncoding(936).GetBytes(TempStr + Suffix).Length);
                                                }
                                            else
                                                {
                                                TCPClientStream.Write(System.Text.Encoding.UTF8.GetBytes(TempStr + Suffix), 0,
                                                    System.Text.Encoding.UTF8.GetBytes(TempStr + Suffix).Length);
                                                }
                                            }
                                        AddText("客户端从本地端口 " + FC.GetLocalPort(TCPClientStation) + " 成功发送到服务器 " +
                                        TargetServerIPAddress + " 的端口： " + TargetServerPort + "，文件名称：" + newOpenFile.FileNames[a]);
                                        }
                                    catch (Exception ex)
                                        {
                                        AddText(ex.Message);
                            ...
                            if (Clients != null)
                                ...
                                for (int b = 0; b < Clients.Length; b++)
                                    {
                                    Clients[b].Suffix = Suffix;
                                    if (ClientSendMessageInHEX == true)
                                        {
                                        //前面已经转换为16进制，所以这里就取消
                                        //Clients[b].SendMessageInHEX = true;
                                        if (Clients[a].Send(TempStr) == true)
                                            {
                                            AddText("客户端 " + (a + 1) + " 从本地端口 " + Clients[a].ClientPort +
                                            " 成功发送到服务器 " + Clients[a].ServerIPAddress
                                            + " 的端口： " + Clients[a].ServerListenPort + "，文件名称：" + newOpenFile.FileNames[a]);
                                            }
                                        else
                                            {
                                            AddText("客户端 " + (a + 1) + "发送文件失败...");
                                            }
                                        }
                                    else
                                        {
                                        if (GB2312Coding == true)
                                            {
                                            Clients[b].GB2312Coding = true;
                                            }
                                        if (Clients[b].Send(TempStr) == true)
                                            {
                                            AddText("客户端 " + (a + 1) + " 从本地端口 " + Clients[a].ClientPort +
                                                " 成功发送到服务器 " + Clients[a].ServerIPAddress
                                                + " 的端口： " + Clients[a].ServerListenPort + "，文件名称：" + newOpenFile.FileNames[a]); 
        ...
        private void btnMultiListen_Click(object sender, EventArgs e)
            ...
            try
                ...
                if (StartMultiConnect == true)
                        ...
                        if (Clients != null)
                            ...
                            for (int a = 0; a < Clients.Length; a++)
                                ...
                                try
                                    ...
                                    Clients[a].Close();
                                    Clients[a] = null;
                                    AddText("成功关闭客户端连接服务器端口线程：" + (a + 1));
                                    }
                                catch (Exception ex)
                                    {
                                    AddText("关闭客户端连接服务器端口线程 " + (a + 1) + " 错误：" + ex.Message);
                                    }
                                }
                    ...
                    MultiServerConnectThread=null;
                    MultiServerConnectThread=new Thread(MonitorMultiConnect);
                    MultiServerConnectThread.IsBackground=true;
                    MultiServerConnectThread.Start();

                    //0~4: None, LF, CR, CR+LF,CustomizedEnding
                    TempCustomSuffix = new string[ClientListView1.CheckedItems.Count];

                    Clients = new SimpleTCPIPClient[ClientListView1.CheckedItems.Count];
                    for (int a = 0; a < Clients.Length; a++)
                        {
                        try
                            {
                            Clients[a] = new SimpleTCPIPClient(ClientListView1.CheckedItems[a].SubItems[1].Text,
                            Convert.ToUInt16(ClientListView1.CheckedItems[a].SubItems[2].Text),"彭东南");
                            ...
                            switch (ClientListView1.CheckedItems[a].SubItems[3].Text)
                                ...
                                case "CUS":
                                    ...
                                    Clients[a].Suffix = txtCustomizedEndingCodeForClient.Text;
                                    break;

                                }

                            Clients[a].Start();
                            AddText("启动连接多服务器/端口 " + (a + 1) + " 线程成功...");
                            }
                        catch (Exception ex)
                            {
                            AddText("启动连接多服务器/端口 " + (a + 1) + " 线程错误：" + ex.Message);
                            }
                        }
        ...
        private void btnConnectToServer_Click(object sender, EventArgs e)
            ...
            try
                ...
                //if (ClientConnectToServerThread == null) 
                //    {
                ClientConnectToServerThread = new Thread(ClientReadMessageFromServer);
                            ...
                            if (Clients != null)
                                ...
                                for (int b = 0; b < Clients.Length; b++)
                                    ...
                                    if (ClientSendMessageInHEX == true)
                                        ...
                                        if (Clients[b].Send(TempStr + Suffix) == true)
                                            {
                                            AddText("客户端 " + (b + 1) + " 从本地端口 " + Clients[b].ClientPort + " 成功发送到服务器 " +
                                                Clients[b].ServerIPAddress + " 的端口： " + Clients[b].ServerListenPort + "，文件名称：" + newOpenFile.FileNames[a]);
                                            }
                                        else 
                                            {
                                            AddText("客户端 " + (b + 1) + " 发送文件失败");
                                            }
        ...
        /// 实例化函数
        /// </summary>
        public TCPClientForm(string DLLPassword)
            {
        ...
        private void AddAvailableIPAddressToList() 
            ...
            try
                ...
                if (IPAddressList == null)
                    ...
                    FeedBackClientParameters[0].TargetServerPort = 8888;
                ...
                if (IPAddressList.Length > 1)
                    ...
                    FC.AddRichText(ref rtbTCPIPHistory, "Found IP list:");
                    ...
                    for (int a = 0; a < IPAddressList.Length; a++)
                        ...
                        FC.AddRichText(ref rtbTCPIPHistory, IPAddressList[a]);
        ...
        private void LanguageForUserInterface() 
            ...
            if (ChineseLanguage == true)
                {
                this.Text = "以太网客户端通讯软件";
                SavePara_ToolStripMenuItem.Text = "保存参数";
                Client_GroupBox1.Text = "客户端";
                ...
                this.Text = "TCP/IP Client Communication Software";
                SavePara_ToolStripMenuItem.Text = "Save Para";
                Client_GroupBox1.Text = "Be Client";
                ...
                try
                        ...
                        while (TCPClientStation.Connected == true)
                            ...
                            if (bytes == 0)
                                ...
                                AddText("Client port " + FC.GetLocalPort(TCPClientStation) + " from server IP: " 
                                ...
                                ErrorMessage = "Client port " + FC.GetLocalPort(TCPClientStation) + " got: " + TempRecord;
                                }
                            }
        ...
        /// <summary>
        /// 客户端开始与服务器进行多端口通讯的读取线程
        /// </summary>
        private void MonitorMultiConnect()
            {
            while (true)
                {
                try
                    {
                    if (StartMultiConnect)
                        {
                        if (Clients != null)
                            {
                            for (int a = 0; a < Clients.Length; a++)
                                {
                                if (Clients[a].ReceivedString != "")
                                    {
                                    FC.UpdateRichTextBox(ref rtbCurrentReceived, Clients[a].ReceivedString);
                                    AddText("客户端本地端口号：" + Clients[a].ClientPort + ", 从服务器：" + 
                                        Clients[a].ServerIPAddress + ",端口 " + Clients[a].ServerListenPort + " 收到: "
                                        + Clients[a].ReceivedString);
                                    Clients[a].ReceivedString = "";
                                    //AddText("服务器IP地址为：" + Clients[a].ServerIPAddress);
                                    //AddText("客户端本地端口号为：" + Clients[a].ClientPort);
                                    }
                                }
                            }
                        }
